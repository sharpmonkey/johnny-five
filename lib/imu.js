  ...
  L3G4200D: {
    ...
    REGISTER: {
      value: {
        ...
        SETUP: [0x20, 0x0F],
        // CTRL_REG4
        // 0bAxBCxxxx A is BDU, BC drive scale, must be 100Hz (gyro.js is using 100hz for integration)
        SCALE250: [0X23, 0x00],
        SCALE500: [0X23, 0x10],
        SCALE2000: [0X23, 0x30],
        // STATUS_REG
    ...
    initialize: {
      value: function(board, opts) {
        ...
        if(opts.sensitivity == 2000) {
          io.i2cWrite(address, this.REGISTER.SCALE2000);
        } else if(opts.sensitivity = 500) {
          io.i2cWrite(address, this.REGISTER.SCALE500);
        } else {
          io.i2cWrite(address, this.REGISTER.SCALE250);
        }
        ...
        io.i2cRead(address, this.REGISTER.READ, READLENGTH, function(data) {
          ...
          var zda = 0x08;
          var zor = 0x80;
          // check if new z data available and no overrun (ZDA ZOR)
          //console.log(status, status & zda, status & zor);
          //if((status & zda) > 0 && (status & zor) > 0) {
            var x = int16(data[2], data[1]);
            var y = int16(data[4], data[3]);
            var z = int16(data[6], data[5]);
    ...
    identifier: {
      value: function(opts) {
        var address = opts.address || Drivers["L3G4200D"].ADDRESSES.value[0];
        return "L3G4200D-" + address;
      }
    }
  }
};
...
var Controllers = {
  ...
  GY80: {
    ...
    components: {
      value: ["accelerometer","gyro", "barometer", "temperature"/*, "compass" */]
    },
    accelerometer: {
      get: function() {
        return priv.get(this).accelerometer;
...
Controllers["GY-80"] = Controllers.GY80;

  ...
  L3G4200D: {
    initialize: {
      value: function(opts, dataHandler) {
        ...
        var state = priv.get(this),
          driver = IMU.Drivers.get(this.board, "L3G4200D", opts);

        state.sensitivity = opts.sensitivity || 250;

        // based on http://www.forkrobotics.com/wp-content/uploads/2013/05/L3G4200D.pdf
        // Page 10, table Mechanical characteristics @ Vdd = 3.0 V, T = 25 °C
        // Sensitivity: 250  dps => 0.00875 dps/digit
        // Sensitivity: 500  dps => 0.01750 dps/digit
        // Sensitivity: 2000 dps => 0.07000 dps/digit
        if(state.sensitivity == 2000) {
          state.factor = 0.07;
        } else if(state.sensitivity == 500) {
          state.factor = 0.0175;
        } else {
          state.factor = 0.00875;
        }
    ...
    toNormal: {
      value: function(raw) {
        var state = priv.get(this);
        return  raw * state.factor;
      }
    },
    toDegreesPerSecond: {
      value: function(raw, rawCenter) {
        return (this.toNormal(raw) - this.toNormal(rawCenter));
      }
    }
  }
};
...
Controllers["GY80"] = Controllers.L3G4200D;
